...
router.get('/',async (req, res) =>  {
    if (req.session.userid != 0)
    {
        ...
        if (rowss.length){
            ...
            ", qr_country, qr_gender, qr_age, qr_fame, qr_distance, qr_sort, qr_order, ";");
            ...
            if (rows.length) {
                if (arr_tags.length && tag_search) {
                    ...
                    if (new_rows.length)
                        ...
                        res.render('search', { title: 'Search | No User Found', sort, order, distance_range, age_0, age_1, common_count, fame, tag_search, country, notif});
                }
                    if (sort == 'Common Tags') {
                        var index;
                        var rw = [];
                        ...
                        for (let i = 0; i < rows.length; i++) {
                            ...
                            if (tags.length >= common_count) {
                                ...
                                function render0() {
                                    ...
                                    res.render('search', { title: 'Search', rw, sort, order, distance_range, age_0, age_1, common_count, fame, tag_search, country, notif});
                                }
