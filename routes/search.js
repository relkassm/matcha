...
router.post('/', async (req, res) => {
    ...
    if (req.body.like) {
        ...
        if (check_match.length)
            var [block] = await connection.execute("INSERT INTO matcha.match (id_user0, id_user1, time) VALUES(?, ?, now());", [req.body.id, connected.id]);
    }
    if (req.body.block) {
        const [block] = await connection.execute("INSERT INTO matcha.block (blocker, blocked) VALUES(?, ?);", [connected.id, req.body.id]);
    }
    if (req.body.report) {
